...
class CreateFeriadoTable extends Migration {
	...
	public function up()
	{
		Schema::create('feriado', function(Blueprint $table)
		{
			...
			$table->integer('id_pais')->index('fk_feriado_pais_idx');
			$table->integer('id_estado')->nullable()->index('fk_feriado_estado_idx');
			$table->integer('id_cidade')->nullable()->index('fk_feriado_cidade_idx');
			$table->date('data');
		});
	}
